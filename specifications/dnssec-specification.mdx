...
<Note type="info">
**Working Implementation**: This document describes three complementary DNSSEC P-256 demonstration projects deployed on Sepolia, all leveraging the EIP-7951 P-256 precompile. Working implementations are available in the [`dnssec-solutions`](../../dnssec-solutions) repository.
</Note>
...
This document provides a comprehensive technical reference for implementing DNSSEC P-256 (Algorithm 13) as a profile in the Universal Resolver Matrix (URM), expanding the normative specification from the proposed **ENSIP-XX** (Ethereum Name Service Improvement Proposal for standardizing DNSSEC onchain resolution).

### 1.2. Demonstration Projects

This specification is supported by three demonstration projects in the [`dnssec-solutions`](../../dnssec-solutions) repository that showcase different approaches to DNSSEC verification:
